...
class AttendanceSerializer(serializers.ModelSerializer):
    ...
    geofence_verified = serializers.SerializerMethodField()
    ...
    def get_geofence_verified(self, obj):
        """Return True/False/None if the attendance coordinates are inside the
        farm's geofence.

        Priority:
          1. Check-out coordinates (latest known location at end of shift)
          2. Check-in coordinates (start of shift)
          3. None if no coordinates or no farm

        None also means no geofence is configured.
        """
        if obj.farm_id is None:
            return None
        from apps.gps.utils import location_inside_farm

        # Prefer check-out coords (latest known location)
        if obj.check_out_lat is not None and obj.check_out_lng is not None:
            return location_inside_farm(
                obj.farm,
                float(obj.check_out_lat),
                float(obj.check_out_lng),
            )
        # Fall back to check-in coords
        if obj.check_in_lat is not None and obj.check_in_lng is not None:
            return location_inside_farm(
                obj.farm,
                float(obj.check_in_lat),
                float(obj.check_in_lng),
            )
        return None
